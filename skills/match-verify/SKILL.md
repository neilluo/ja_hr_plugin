...
3. **`needs_review:["years"]` 的候选人必须复核工作年限**（D13）：用 `evidence.work_text` 原文逐段累加重算，修正值回填 `candidate_overrides.years_experience`；原文支撑不了 → 不拿估算值当年限否决依据，该岗从宽记「待定」并在 evidence 注明。**P5 身份安全阀**：`needs_review` 含 `"name"`/`"email"`（姓名来自文件名/OCR/agent 草稿，或邮箱命中 OCR 噪声规则如 qq.com→q9.com）→ 对照 `evidence.name_text`/`email_text` 原文逐字比对，复核结论写进该人任一条目 evidence（`candidate_overrides` 无 name/email 键，误读业务话照转请用户人工修正）；`evidence.location_text` 是期望地点命中行原文，串栏垃圾值按 D14 覆盖或注明。
...
5. **组织**：`org_confidence=="low"` 由 agent 依 evidence 判定并回填 `candidate_overrides.org` + `org_reason`；**判不了才问用户，不猜**。**P5 预筛错杀阀**：分片候选人带 `prefilter_suspicious`（被组织预筛删除、但学历/年限/证书**全部机械达标**的跨组织岗位清单）或 `needs_review` 含 `"org"` 的同样必须依 evidence 复核组织——确认无误写 `org_reason`；确认判错写 `org` + 按清单 `job_key` 合并 reject 新组合（reason 注明待重切补判），apply 写回后**重跑同一条生成判定输入命令**（脚本以库内组织为准重切分片）再补判、再 apply（细则见 resume-intake/HOTPATH.md 回合 3 规则 5）。
