...
- 解析失败（扫描件/加密/乱码）如实告知并建议提供文字版，**绝不硬造字段**；失败项不隐藏。
...
## 幂等与续跑（D12，checkpoint 语义按契约 v3 §9#6 加强）
...
- 用户要求"从头重来"时才加 `--reset`（会清空断点；已写入表格的记录按查重主键覆盖更新，不会产生重复记录）。

## 批量与边界

- 同一文件 MD5 与库内已有附件相同 → 重复上传，跳过并告知"与系统内已有附件完全相同"。
...
- **技能标签/部门/地点等新选项（2026-09-17 起根治口径）**：`shared/aitable_io.py` 的 `ensure_options` 已改为**只读 + 延迟补建**——只 `field get` 读回现有选项池（供报告）并把缺失名记入 `pending_options`，**彻底移除了 `dws aitable field update` 路径**；实际补建由 `record create/update/upsert` 直接写**选项名**时服务端自动完成（自动补入缺失选项、**不动已有选项 id**，单字段选项上限 3000）。intake/apply 各脚本均已按此运行，agent 无需（也不允许）另行补选项。
