...
4. **低置信组织复核**：`org_confidence == "low"` 的候选人，依 `evidence` 原文判定组织（制造基地/厂务/设备/EHS → 制造中心；财务/行政/人力/数据信息 → 职能中心）；**判不了才问用户，不猜**。判定结果记录下来，在匹配环节的 `candidate_overrides` 里回填（见 [定向匹配](../match-verify/SKILL.md)）；`needs_review` 含 `"years"` 的候选人同样留到匹配回合用原文复核工作年限。**P5 起身份/组织安全阀（同留到匹配回合处置，本回合只照转 warnings）**：脚本给 evidence 补 `name_text`/`email_text`/`location_text` 命中行原文，姓名来源文件名/OCR/agent 草稿或邮箱疑似 OCR 噪声（qq.com→q9.com 等）→ `needs_review` 追加 `"name"`/`"email"`；组织预筛删掉「学历/年限/证书全部机械达标」的跨组织岗位 → 分片候选人带 `prefilter_suspicious` 且 `needs_review` 追加 `"org"`（组织判错会整池静默错杀，必须在匹配回合依 evidence 复核，判错走 `candidate_overrides.org` + 重跑，见 [HOTPATH.md](HOTPATH.md) 回合 3 规则 5）。
