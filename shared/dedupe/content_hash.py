# -*- coding: utf-8 -*-
"""库内附件去重：键 = **附件内容真 MD5**（内容级比对）。

索引与判定按优先级：(1) 本地文件真 MD5 命中库内「附件内容MD5」→ 判重复，跳过；
(2) (文件名, 大小) 命中且库内那条有哈希但内容不同 → 不判重复，走覆盖更新；
(3) (文件名, 大小) 命中但库内那条没有哈希 → 按老键回退判重复并告警。
"""
...
class ContentHashDeduper(NameSizeDeduper):
    ...
    def legacy_warning(self) -> List[str]:
        """老记录告警（库里有早期写入的记录时给一条，说明回退口径与收敛方式）。"""
