# -*- coding: utf-8 -*-
"""P5 身份字段安全阀的纯函数工具（intake_resume.py / build_match_input.py 共用）。

背景（W6 召回审计）：name / email / expected_location 在 digest 里**既无 needs_review
标记也无原文保留面**，正则/OCR 错抽即静默入库（实测盲区：OCR 垃圾姓名「本汉族」、
OCR 邮箱 qq.com→q9.com、文件名覆盖正文姓名「林云/林之府」、双栏串栏期望地点）。
本模块提供三件确定性能力，全部零第三方依赖、python 3.9/3.14 兼容（契约 D18）：

1. `identity_evidence(text, ...)`：从简历全文里取「字段命中所在行原文」（≤60 字），
   进 digest 的 evidence.name_text / email_text / location_text——给 Turn 2 一个
   可见的原文复核面（对齐 D4 对 education_text 的保留思路）。
2. `email_ocr_noise(email)`：邮箱 OCR 噪声规则判据（域名无点 / TLD 含非字母 /
   域名主体字母+数字混排，如 q9.com ← qq.com、gmai1.com ← gmail.com、163.c0m ←
   163.com）。命中 → needs_review 追加 "email"。规则清单同步列在 P5 报告。
...
def identity_evidence(text: Any, name: Any = None, email: Any = None,
    ...
    """三个身份字段的「命中行原文」evidence（P5 安全阀的原文保留面）。
...
def email_ocr_noise(email: Any) -> Optional[str]:
    """邮箱 OCR 噪声判据（P5 设计第 2 条，规则清单同步列在 P5 报告）。
