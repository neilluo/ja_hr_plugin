# -*- coding: utf-8 -*-
"""岗位侧业务预判规则（纯函数对象，无 IO）。

JobOrgPolicy 是 guess_org 的 B 变体——三段式（①文件名/正文显式声明 ②部门+岗位名
关键词 ③正文兜底）、low 置信度仍写库（组织留空 = 该岗位匹配不到任何简历，留空是
硬功能故障）。与简历侧 A 变体（两段式、low 不写库）策略相反，不共享关键词表。
"""
