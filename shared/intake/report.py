# -*- coding: utf-8 -*-
"""IntakeReport：报告统计判定 + 组装 + 产物落盘 + 尾部人读清单。
...
checkpoint 的终稿 update 与非原子落盘**不在本类**（CheckpointStore 的
...
class IntakeReport:
    ...
    def assemble(self, args: Any, budget: float, t_start: float, counter: Any,
        ...
        turns_saved = (max(0, n_done_files * self.old_turns_per_file - self.new_turns)
                       if files else 0)
        # 基础设施级失败判定：**该写的都没写进去** → ok=false，让 agent 重跑本步。
        ...
        # 闸门口径：升级自 OCR 的条目（有文字但姓名低置信度）不计入闸门统计，
        # 它们不是「整批读不出文字」的格式问题。
        self.n_gate_vision = sum(
            1 for e in entries
            if e.get("parse_status") == "needs_agent_vision"
            and not e.get("escalated_from_ocr"))
        self.n_gate_entries = sum(
    ...
    def emit(self, extract_ms: int, checkpoint_path: Path,
        ...
        if self.partial:
            if self.vision_gated:
                c.partial_vision_gate(self.n_gate_vision, self.n_gate_entries,
