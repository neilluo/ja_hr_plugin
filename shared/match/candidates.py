...
这些 dict 是 normalize() 新造的编排层自有对象，就地修改被编排层封闭。
"""

import re
from typing import Any, Dict, List, Optional

from fields.identity import (           # 身份安全阀判据（纯函数，零第三方依赖）
...
from match.match_basics import DEFAULT_LOCATION
from match.tablevalues import WORK_TEXT_LIMIT, SKILL_TEXT_LIMIT, as_list, \
    clean_ws, clip, dedupe_keep_order, full

#: candidates 元素的字段清单（只增不减：C1 多给的字段原样透传）
#: 只增：email（身份阀的判据与原文面要用）/ name_source / parse_backend（姓名复核判据）
...
# --- evidence 兜底：分段是按标题正则切的，真实简历里有些切不出
#     「教育经历」段或「工作经历」段（简历根本没有这些标题）。
...
class CandidateNormalizer:
    """candidates.json 元素 → digest candidates 元素（纯计算）。"""

    def build_evidence(self, cand: Dict[str, Any]) -> Dict[str, str]:
        """education_text / cert_text **完整保留**；work_text / skill_text 可截断。

        兼容 C1 把原文段放在 `evidence` 里或放在 `sections` 里。
        身份字段原文行 name_text / email_text / location_text（≤60 字）——
        上游给了就透传（再封顶一次），没给先置空串，有全文时由 enrich_evidence 兜底补。
        """
    ...
    def enrich_evidence(self, cand: Dict[str, Any], full_text: str) -> List[str]:
        ...
        **就地**写 cand["evidence"] / cand["evidence_provenance"] / cand["evidence_chars"]
        ...
        for key, hint, before, after, limit in plan:
            ...
            if not win:
                continue
            # 教育/证书段：兜底窗口也**不截断**；work/skill 段按各自上限截断
        ...
        # 身份原文行兜底——旧版 candidates.json / 模式 B 可能没带 name_text 等三键，
        ...
        if any(not (ev.get(k) or "").strip() for k in IDENTITY_EVIDENCE_KEYS):
            ...
            if clean_ws(loc_raw) == DEFAULT_LOCATION:
                loc_raw = None            # 「不限」是兜底值，不是原文词，不拿它去搜行
    ...
    def years_source_of(self, cand: Dict[str, Any]) -> Optional[str]:
        """输出的键名是 `years_experience_source`，也认 `years_source`。"""
    ...
    def normalize(self, cand: Dict[str, Any], idx: int, warnings: List[str]) -> Dict[str, Any]:
        """把 C1 的 candidates.json 元素归一成 digest candidates 元素。"""
        ...
        # 身份安全阀（判据在 shared/fields/identity.py）：姓名来源 filename/OCR/agent 草稿
        # → needs_review 追加 "name"；邮箱命中 OCR 噪声规则 → 追加 "email"。只标记不拦截；
        # 逐人 warning 会按分片数复制、撑大 shard 体积，这里把判据记在
        ...
        if trunc["education_text"] or trunc["cert_text"]:
            ...
            warnings.append("%s(%s)：education_text/cert_text 被上游截断，"
    ...
    def aggregate_identity_review(self, active: List[Dict[str, Any]],
        ...
        """身份复核聚合 warning（逐人判据在 normalize 的临时键里；聚合是
        为了控制分片体积——meta.warnings 会按分片数复制。复核动作由 needs_review +
        evidence.name_text/email_text 驱动）。
        """
