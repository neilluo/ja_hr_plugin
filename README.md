...
三步走：Turn 1 `intake_resume.py --files ...`（提取+预抽字段+去重（本批内真 MD5、库内附件按文件名+字节大小）+手机号批量查重+批量写+技能标签只增不删+期望地点兜底「不限」+并发原文件名附件+回读；checkpoint 幂等续跑）→ Turn 2 agent 审阅报告（转述清单、扫描件/加密如实告知不硬造、手机号冲突停下问用户、低置信组织复核）→ Turn 3 默认接续定向匹配。
